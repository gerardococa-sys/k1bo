...
export default async function AdminClienteSolicitudesPage({
  params,
}: {
  params: { id: string }
}) {
  ...
  const { data: solicitudes } = await supabase
    ...
    .select(`
      ...
      professional:professionals!quote_requests_professional_id_fkey(
        profiles(full_name)
      )
    `)
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: '#F2F0ED', borderBottom: '1px solid #2C2C2C10' }}>
              {['Código', 'Profesional', 'Categoría', 'Descripción', 'Fecha', 'Estado'].map((h) => (
          ...
          <tbody>
            {rows.map((r: any) => {
              ...
              const proProfiles = r.professional?.profiles
              const proName = Array.isArray(proProfiles) ? proProfiles[0]?.full_name : proProfiles?.full_name
