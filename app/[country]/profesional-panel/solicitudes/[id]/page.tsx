...
const STATUS_BADGE: Record<string, { bg: string; color: string; label: string; description: string }> = {
  pending: {
    ...
    description: 'El propietario está esperando tu respuesta.',
  },
  responded: {
    bg: '#1C141015', color: '#1C1410', label: 'Respondida',
    description: 'Has respondido a esta solicitud.',
  },
  accepted: {
    bg: '#6B7B6E20', color: '#3d4d40', label: 'Aceptada',
    description: 'Has aceptado esta solicitud.',
  },
  rejected: {
    bg: '#1C141010', color: '#6B7B6E', label: 'Rechazada',
    description: 'No pudiste atender esta solicitud.',
  },
}
...
function Initials({ name }: { name?: string }) {
  ...
  return (
    <div style={{
      width: 64, height: 64, borderRadius: '50%', backgroundColor: '#B85C1A15',
...
export default async function ProSolicitudDetailPage({
  params,
}: {
  ...
  return (
    <div className="container mx-auto px-4 py-10 max-w-5xl">
      ...
      <Link
        ...
        style={{ fontFamily: FONT_SANS, fontSize: '15px', color: '#B85C1A', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '6px', marginBottom: '20px' }}
        className="sol-pro-back"
      >
      ...
      {/* 2-col layout */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '20px' }} className="sol-pro-grid">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <Card>
            ...
            <p style={{ fontFamily: FONT_SANS, fontSize: '13px', color: '#B85C1A', textAlign: 'center', margin: 0 }}>
              Propietario
            </p>
      ...
      <style>{`
        .sol-pro-back:hover { text-decoration: underline; }
        @media (min-width: 768px) {
          .sol-pro-grid { grid-template-columns: 2fr 1fr; }
        }
      `}</style>
    </div>
  )
